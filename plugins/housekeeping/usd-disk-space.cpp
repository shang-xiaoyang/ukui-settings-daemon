...
void DIskSpace::usdLdsmGetConfig()
{
    ...
    USD_LOG(LOG_DEBUG,"free_percent_notify:%f,free_size_gb_no_notify:%f",free_percent_notify,free_size_gb_no_notify);
...
bool  DIskSpace::ldsm_mount_has_space (LdsmMountInfo *mount)
{
    ...
    USD_LOG(LOG_DEBUG,"%s:(percent_flag:%d，size_falg:%d),f_frsize:%d,f_bavail:%d,f_blocks:%d",g_unix_mount_get_mount_path(mount->mount),percent_flag,size_falg,
            mount->buf.f_frsize,mount->buf.f_bavail,mount->buf.f_blocks);
