...
bool xrandrConfig::writeFile(const QString &filePath, bool state)
{
    ...
    for (const KScreen::OutputPtr &output : outputs) {
        ...
        xrandrOutput::writeGlobalPart(output, info, nullptr);
        info[QStringLiteral("primary")] = isOneConnectOutput ? true :output->isPrimary(); //
        info[QStringLiteral("enabled")] = isOneConnectOutput ? true : output->isEnabled();

        auto setOutputConfigInfo = [&info](const KScreen::OutputPtr &out , bool isOneConnectOutput) {
            if (!out) {
                USD_LOG(LOG_DEBUG,".");
                return;
            }
            ...
            QVariantMap pos;
            pos[QStringLiteral("x")] = isOneConnectOutput ? 0 : out->pos().x();
            pos[QStringLiteral("y")] = isOneConnectOutput ? 0 : out->pos().y();
            info[QStringLiteral("pos")] = pos;
        };
        setOutputConfigInfo(output->isEnabled() ? output : nullptr,isOneConnectOutput);
